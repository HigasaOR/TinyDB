...
/* constants for splitting a leaf node */
...
/* get the pointer of the next leaf pointer of leaf node*/
uint32_t* leaf_node_next_leaf(void* node) {
    return node + LEAF_NODE_NEXT_LEAF_OFFSET;
}

/* get the pointer of the number of keys of the internal node */
uint32_t* internal_node_num_keys(void* node) {
    return node + INTERNAL_NODE_NUM_KEYS_OFFSET;
}

/* get the pointer of the right child (page number) of the internal node */
uint32_t* internal_node_right_child(void* node) {
    return node + INTERNAL_NODE_RIGHT_CHILD_OFFSET;
}

/* get pointer of the cell of the internal node from given cell number
 * This method is accessed by internal_node_child method.
 */
...
/* get the pointer of the child (page number) of the internal node given the
 * child number */
...
/* get the nth key of the internal node */
uint32_t* internal_node_key(void* node, uint32_t key_num) {
    return (void*)internal_node_cell(node, key_num) + INTERNAL_NODE_CHILD_SIZE;
}

/* determine if the node is root */
...
/* find the position (or the position to insert) of the given key
 * (... < key <= ...)
 */
Cursor* leaf_node_find(Table* table, uint32_t page_num, uint32_t key) {
    ...
    // Binary search (for single node, a stub function now)
...
void leaf_node_split_and_insert(Cursor* cursor, uint32_t key, Row* value) {
    ...
    *node_parent(new_node) = *node_parent(old_node);
    ...
    // next node of new node is the next node of old node
    // next node of old node is the new node
